{%- macro extract_json(json_keys, target_name=target.type) -%}
    {{ return(adapter.dispatch('extract_json')(json_keys, target_name=target.type)) }}
{%- endmacro -%}

{%- macro default__extract_json(json_keys, target_name=target.type) -%}

    {%- if target_name is none -%}

    {%- elif target_name == 'snowflake' -%}

        {%- set field_array = json_keys.split(":") -%}
        {%- set base = field_array[0] -%}
        {# {%- set other_fields = field_array.pop[0] -%}
        {%- set other_fields_text = other_fields|join(':') -%} #}

        {%- do field_array.pop[0] -%}
        ...
        {%- if other_fields_text == '' -%}
            {{ json_keys }}::variant
        {%- else -%}
            {{ base }}::variant:{{ other_fields_text }}
        {%- endif -%}
